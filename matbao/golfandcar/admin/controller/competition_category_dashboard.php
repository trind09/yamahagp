...
$virtual_path = "../assets/competition/";
$id = "";
$name = "";
$description = "";
$image = "";
	...
	if ($message != ""){
		...
		if ($id == ''){
			...
			if (count($error) > 0){
				...
				$sql = "INSERT INTO competition_category (picture, name, description, history) VALUES "
					. "(?,?,?,?)";
			...
			if (count($error) > 0){
				...
				$sql = "UPDATE `competition_category` SET `picture`=?,`name`=?,`description`=?, history = IFNULL(CONCAT(history, '" . $history . "'), '" . $history . "') WHERE id = ?";
				$statement = $pdo->prepare($sql);
...
function ValidateData($post_date){
	...
	if ($name == ""){
		$message .= "Xin điền tên hạng mục</br>";
	}
...
function DeleteRecord($id, $pdo){
	if ($id != ''){
		try{
			$sql = "SELECT * FROM competition_category WHERE id = " . $id;
			...
			$sql = "DELETE FROM competition_category WHERE id = " . $id;
                        ...
                        <div class="card">
                            <form id="clb" action="" method="post" enctype='multipart/form-data'>
                                ...
                                <div class="card-header">
                                    <h2 class="mb-0" id="page-title">Hạng mục thi đấu</h2>
                                ...
                                <div class="card-header">
                                    ...
                                    <div class="form-group">
                                        <label for="name">Tên hạng mục: <span style="color: red;">*</span></label>
                                    ...
                                    <div class="table-responsive">
                                        <table id='example' class='table table-striped table-bordered second dataTable'>
                                            ...
                                            <tbody>
                                                ...
                                                if ($is_admin)
                                                {
													$sql = "SELECT * FROM competition_category ORDER BY create_date DESC LIMIT " . $amount_of_record;
													if ($from_date != "")
													{
														$sql = "SELECT * FROM competition_category where create_date >= '" . $from_date . "' ORDER BY create_date DESC LIMIT " . $amount_of_record;
													}
