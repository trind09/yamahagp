	...
	if ($message != ""){
		...
		if ($id == ''){
			...
			if (count($error) > 0){
				...
				$sql = "INSERT INTO new (image_name, title, description, hyperlink, author) VALUES "
					. "(?,?,?,?,?)";
					$statement = $pdo->prepare($sql);
					$statement->execute(array($image_name, $title, $description, $hyperlink, $author));
			...
			if (count($error) > 0){
				...
				$sql = "UPDATE `new` SET `image_name`=?,`title`=?,`description`=?,`hyperlink`=?, `author`=? WHERE id = ?";
				$statement = $pdo->prepare($sql);
				$statement->execute(array($image_name, $title, $description, $hyperlink, $author, $id));
...
function ValidateData($post_date){
	...
	$hyperlink = strip_tags($post_date['hyperlink']);
	if ($hyperlink == ""){
		$message .= "Xin điền link URL</br>";
	}
	return $message;
}
function BuildUpdateFields($id, $pdo, $domain){
	global $image_tags; global $id; global $title; global $description; global $hyperlink; global $author;
	if ($id != ''){
		$sql = "SELECT * FROM new WHERE id = " . $id;
...
<div class="dashboard-wrapper" id="new_pannel">
                            ...
                            <form id="clb" action="" method="post" enctype='multipart/form-data'>
                                ...
                                <div class="card-header">
									...
									<div class="form-group">
										...
										<textarea class="form-control" id="description" name="author" rows="4" cols="50"><?php echo($author); ?></textarea>
                                    </div>
									<div class="form-group">
                                    ...
                                    <div class="table-responsive">
                                        <table id='example' class='table table-striped table-bordered second dataTable'>
                                            ...
                                            <tbody>
                                                ...
                                                if ($is_admin)
                                                {
													$sql = "SELECT * FROM new ORDER BY create_date DESC LIMIT " . $amount_of_record;
													if ($from_date != "")
													{
														$sql = "SELECT * FROM new where create_date >= '" . $from_date . "' ORDER BY create_date DESC LIMIT " . $amount_of_record;
													}
