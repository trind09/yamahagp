...
if (isset($_POST['insert_update'])){
	...
	if ($message != ""){
		...
		$title_plan = strip_tags($_POST['title_plan']);
		...
		if ($id == ''){
				...
				$sql = "INSERT INTO plan (title_plan,hyperlink, history) VALUES "
					. "(?,?,?)";
					$statement = $pdo->prepare($sql);
					$statement->execute(array( $title_plan, $hyperlink, $history));
			...
			$sql = "UPDATE `plan` SET `title_plan`=?,`hyperlink`=?, history = IFNULL(CONCAT(history, '" . $history . "'), '" . $history . "') WHERE id = ?";
			$statement = $pdo->prepare($sql);
			$statement->execute(array($title_plan, $hyperlink, $id));
...
function ValidateData($post_data){
	$message = "";
	$title_plan = strip_tags($post_data['title_plan']);
	if ($title_plan == ""){
...
function BuildUpdateFields($id, $pdo, $domain){
	global $id; global $title_plan; global $hyperlink;
	if ($id != ''){
		$sql = "SELECT * FROM plan WHERE id = " . $id;
                            ...
                            <form id="clb" action="" method="post" enctype='multipart/form-data'>
                                ...
                                <div class="card-header">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="title_plan" id="title_plan" value="<?php echo($title_plan); ?>" />
                                    </div>
									<div class="form-group">
                                                ...
                                                if ($is_admin)
                                                {
													...
													if (count($result) > 0)
													{
														...
														foreach ($result as $row)
														{
															...
															echo ("<tr>");
															echo ("<td>" . $counter . "</td>" . "
																<td><div class='scrollable'>" . $title_plan . "</div></td>" . "
