...
if (isset($_POST['insert_update'])){
	...
	if ($message != ""){
		...
		if ($id == ''){
				...
				$sql = "INSERT INTO match_schedule (`title`, `description`, `address`, `time`, `history`) VALUES "
					. "(?,?,?,?,?)";
				$statement = $pdo->prepare($sql);
				$statement->execute(array($title, $description, $address, $s_time, $history));
			...
			//Case update, id is always has value
				...
				var_dump($s_time);var_dump('eft');
				$sql = "UPDATE `match_schedule` SET `title`=?,`description`=?,`address`=?,`time`=?, history = IFNULL(CONCAT(history, '" . $history . "'), '" . $history . "') WHERE id = ?";
				$statement = $pdo->prepare($sql);
				$statement->execute(array( $title, $description, $address, $s_time, $id));
...
function ValidateData($post_data){
	...
	$address = strip_tags($post_data['address']);
    if ($address == ""){
		$message .= "Xin điền địa điểm</br>";
	}
	$s_time = strip_tags($post_data['s_time']);
    if ($s_time == ""){
		$message .= "Xin chọn thời điểm</br>";
	}
	return $message;
}
...
function DeleteRecord($id, $pdo){
	if ($id != ''){
		try{
			$sql = "SELECT * FROM match_schedule WHERE id = " . $id;

			$statement = $pdo->prepare($sql);
			$statement->execute();
			$result = $statement->fetchAll(PDO::FETCH_ASSOC);

			$sql = "DELETE FROM match_schedule WHERE id = " . $id;
...
function BuildUpdateFields($id, $pdo, $domain){
	global $id; global $title; global $description; global $address; global $s_time; global $history;
	if ($id != ''){
		$sql = "SELECT * FROM match_schedule WHERE id = " . $id;
                        ...
                        <div class="card">
                            <form id="clb" action="" method="post" enctype='multipart/form-data'>
                                ...
                                <div class="card-header">
                                    <h2 class="mb-0" id="page-title">Lịch trình thi đấu</h2>
                                ...
                                <div class="card-header">
									...
									<div class="form-group">
                                        <label for="address">Địa điểm: <span style="color: red;">*</span></label>
                                        <input type="text" class="form-control" name="address" id="address" value="<?php echo($address); ?>" />
                                    </div>
									<div class="form-group">
                                        <label for="s_time">Thời điểm: <span style="color: red;">*</span></label>
                                        <input type="text" class="form-control" name="s_time" id="datetimepicker" value="<?php echo($s_time); ?>" />
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="table-responsive">
                                        <table id='example' class='table table-striped table-bordered second dataTable'>
                                            ...
                                            <tbody>
                                                ...
                                                if ($is_admin)
                                                {
													$sql = "SELECT * FROM match_schedule ORDER BY create_date DESC LIMIT " . $amount_of_record;
													if ($from_date != "")
													{
														$sql = "SELECT * FROM match_schedule where create_date >= '" . $from_date . "' ORDER BY create_date DESC LIMIT " . $amount_of_record;
													}
													...
													if (count($result) > 0)
													{
														...
														foreach ($result as $row)
														{
															...
															echo ("<td>" . $counter . "</td>" . "
																...
																<td><div class='scrollable'>" . $address . "</div></td>" . "
																<td><div class='scrollable'>" . $s_time . "</div></td>" . "
