	...
	if ($message != ""){
		...
		if ($id == ''){
			...
			if (count($error) > 0){
				...
				$sql = "INSERT INTO event (picture, category, title, description, history) VALUES "
					. "(?,?,?,?,?)";
				$statement = $pdo->prepare($sql);
				$statement->execute(array($picture, $category, $title, $description, $history));
			...
			if (count($error) > 0){
				...
				$sql = "UPDATE `event` SET `picture`=?,`category`=?,`title`=?,`description`=?, history = IFNULL(CONCAT(history, '" . $history . "'), '" . $history . "') WHERE id = ?";
                $statement = $pdo->prepare($sql);
				$statement->execute(array($picture, $category, $title, $description, $id));
...
function BuildUpdateFields($id, $pdo, $domain){
	global $image; global $id; global $title; global $category; global $description; global $history;
	if ($id != ''){
		$sql = "SELECT * FROM event WHERE id = " . $id;
                            ...
                            <form id="clb" action="" method="post" enctype='multipart/form-data'>
                                ...
                                <div class="card-header">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="father_category" id="father_category" class="form-control">
                                            <?php 
												$arr = array('EVENT HIGHTLIGHTS', 'LINE UP');
                                                $i = 1;
												foreach ($arr as $value) {
													if($value == $category){
														echo("<option selected thai='child_category_div".$i."'>" . $value . "</option>");
													} else {
														echo("<option thai='child_category_div".$i."'>" . $value . "</option>");
													}
                                                    $i++;
												}
                                            ?>
                                    ...
                                    <div class="form-group child_category_class" id="child_category_div2"  style="display:none">
                                        <label for="category">Thể loại</label>
                                        <select name="child_category" id="child_category" class="form-control">
											<?php 
												$arr = array('RAPER', 'DJ', 'MC');
												foreach ($arr as $value) {
													if($value == $category){
									...
									<div class="form-group">
                                        <label for="upload">Hình ảnh sự kiện: </label>
                                    ...
                                    <div class="table-responsive">
                                        <table id='example' class='table table-striped table-bordered second dataTable'>
                                            ...
                                            <tbody>
                                                ...
                                                if ($is_admin)
                                                {
													$sql = "SELECT * FROM event ORDER BY create_date DESC LIMIT " . $amount_of_record;
													if ($from_date != "")
													{
													...
													if (count($result) > 0)
													{
														...
														foreach ($result as $row)
														{
															...
															echo ("<tr>");
															echo ("<td>" . $counter . "</td>" . "
																<td><div class='scrollable'>" . $category . "</div></td>" . "
